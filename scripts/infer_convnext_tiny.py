...
class InferenceConfig:
    ...
    if is_kaggle:
        ...
        submission_file: str = 'submission_convnext_tiny.csv'
    else:
        base_path: Path = Path('./datasets/csiro-biomass')
        test_csv: Path = field(init=False)
        test_image_dir: Path = field(init=False)
        model_dir: Path = Path('./outputs/convnext_tiny') # Directory where trained models are stored
        submission_file: str = 'submission_convnext_tiny.csv'

    # --- Model settings (must match training) ---
    model_name: str = 'convnext_tiny' # Backbone model to use
